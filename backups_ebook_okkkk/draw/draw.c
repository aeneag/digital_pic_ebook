...
int SetTextDetail(char *pcHZKFile, char *pcFileFreetype, unsigned int dwFontSize)
{
	...
	while (ptFontOpr)
	{
		...
		{
			iError = ptFontOpr->Font_init(pcFileFreetype, dwFontSize);
		}
...
{
	int iError;
	g_ptDispOpr = Get_show_ops(pcName);
	if (!g_ptDispOpr)
	{
		return -1;
	}

	iError = g_ptDispOpr->Device_init();
	return iError;
}
	...
	if (ptFontBitMap->iBpp == 1)
	{
		for (y = ptFontBitMap->iYTop; y < ptFontBitMap->iYMax; y++)
		{
			...
			for (x = ptFontBitMap->iXLeft, bit = 7; x < ptFontBitMap->iXMax; x++)
			{
				...
				if (ucByte & (1<<bit))
				{
					g_ptDispOpr->Show_pixel(x, y, COLOR_FORE_GROUND);
				}
				else
				{
					/* 使用背景色, 不用描画 */
	...
	while (1)
	{
		...
		while (ptFontOpr)
		{
			...
			if (0 == iError)
			{
				...
				{
					/* 首先清屏 */
					g_ptDispOpr->Clean_screen(COLOR_BACK_GROUND);
					bHasNotClrSceen = 0;
				}
				DBG_PRINTF("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
				/* 显示一个字符 */

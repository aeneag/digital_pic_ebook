...
#define DIR_FILE_ICON_HEIGHT   DIR_FILE_ICON_WIDTH//40
#define DIR_FILE_NAME_HEIGHT   20
#define DIR_FILE_NAME_WIDTH   (DIR_FILE_ICON_HEIGHT + DIR_FILE_NAME_HEIGHT)//60
...
static T_Page_Layout g_tBrowsePageMenuIconsLayout = {
        .iMaxTotalBytes = 0,
...
static T_Page_Layout g_tBrowsePageDirAndFileLayout = {
        .iMaxTotalBytes = 0,
...
/* 用来描述某目录里的内容 */
static P_Dir_Content *g_aptDirContents;  /* 数组:存有目录下"顶层子目录","文件"的名字 */
...
static T_Pixel_Datas g_tDirClosedIconPixelDatas;
static T_Pixel_Datas g_tDirOpenedIconPixelDatas;
static T_Pixel_Datas g_tFileIconPixelDatas;
...
/* 计算菜单中各图标坐标值 */
static void  CalcBrowsePageMenusLayout(P_Page_Layout ptPageLayout)
{
    int iWidth;
    int iHeight;
...
static int BrowsePageGetInputEvent(P_Page_Layout ptPageLayout, P_Input_event ptInputEvent)
{
    return Generic_get_input_event(ptPageLayout, ptInputEvent);
}

static int GetInputPositionInPageLayout(P_Page_Layout ptPageLayout, P_Input_event ptInputEvent)
{
    int i = 0;
 ...
 */
static int GenerateBrowsePageDirAndFile(int iStartIndex, int iDirContentsNumber, P_Dir_Content *aptDirContents, P_Video_Mem ptVideoMem)
{
    int iError;
    int i, j, k = 0;
    int iDirContentIndex = iStartIndex;
    P_Page_Layout ptPageLayout = &g_tBrowsePageDirAndFileLayout;
    P_Layout atLayout = ptPageLayout->atLayout;

    Clear_rectangle_in_video_mem(ptPageLayout->iTopLeftX, ptPageLayout->iTopLeftY, ptPageLayout->iBotRightX, ptPageLayout->iBotRightY, ptVideoMem, COLOR_BACK_GROUND);

    Set_font_size(atLayout[1].iBotRightY - atLayout[1].iTopLeftY - 5);
    ...
    for (i = 0; i < g_iDirFileNumPerCol; i++)
    {
        for (j = 0; j < g_iDirFileNumPerRow; j++)
        {
            if (iDirContentIndex < iDirContentsNumber)
            {
                ...
                {
                    Pic_merge(atLayout[k].iTopLeftX, atLayout[k].iTopLeftY, &g_tDirClosedIconPixelDatas, &ptVideoMem->tPixelDatas);
                }
                else
                {
                    Pic_merge(atLayout[k].iTopLeftX, atLayout[k].iTopLeftY, &g_tFileIconPixelDatas, &ptVideoMem->tPixelDatas);
                }

                k++;
                /* 显示目录或文件的名字 */
                //DBG_PRINTF("MergerStringToCenterOfRectangleInVideoMem: %s\n", aptDirContents[iDirContentIndex]->strName);
                iError = Merger_string_to_center_of_rectangle_in_video_mem(atLayout[k].iTopLeftX, atLayout[k].iTopLeftY, atLayout[k].iBotRightX, atLayout[k].iBotRightY, (unsigned char *)aptDirContents[iDirContentIndex]->strName, ptVideoMem);
...
static int GenerateDirAndFileIcons(P_Page_Layout ptPageLayout)
{
    ...
    g_tDirClosedIconPixelDatas.auc_pixel_datas = malloc(ptPageLayout->iMaxTotalBytes);
    if (g_tDirClosedIconPixelDatas.auc_pixel_datas == NULL)
    {
        return -1;
    }

    g_tDirOpenedIconPixelDatas.iBpp          = iBpp;
    g_tDirOpenedIconPixelDatas.auc_pixel_datas = malloc(ptPageLayout->iMaxTotalBytes);
    if (g_tDirOpenedIconPixelDatas.auc_pixel_datas == NULL)
    {
        return -1;
    }

    g_tFileIconPixelDatas.iBpp          = iBpp;
    g_tFileIconPixelDatas.auc_pixel_datas = malloc(ptPageLayout->iMaxTotalBytes);
    if (g_tFileIconPixelDatas.auc_pixel_datas == NULL)
    {
        return -1;
    }
    ...
    /* 从BMP文件里提取图像数据 */
    /* 1. 提取"fold_closed图标" */
    iError = Get_pixel_datas_for_icon(g_strDirClosedIconName, &tOriginIconPixelDatas);
    if (iError)
    {
    ...
    /* 2. 提取"fold_opened图标" */
    iError = Get_pixel_datas_for_icon(g_strDirOpenedIconName, &tOriginIconPixelDatas);
    if (iError)
    {
    ...
    /* 3. 提取"file图标" */
    iError = Get_pixel_datas_for_icon(g_strFileIconName, &tOriginIconPixelDatas);
    if (iError)
    {
        DBG_PRINTF("GetPixelDatasForIcon %s error!\n", g_strFileIconName);
        return -1;
...
static void ShowBrowsePage(P_Page_Layout ptPageLayout)
{
    ...
    /* 1. 获得显存 */
    ptVideoMem =Get_video_mem(ID("browse"), 1);
    if (ptVideoMem == NULL)
    {
    ...
    /* 如果还没有生成"目录和文件"的图标 */
    if (!g_tDirClosedIconPixelDatas.auc_pixel_datas)
    {
...
static void SelectDirFileIcon(int iDirFileIndex)
{
    ...
    {
        Pic_merge(g_atDirAndFileLayout[iDirFileIndex].iTopLeftX, g_atDirAndFileLayout[iDirFileIndex].iTopLeftY, &g_tDirOpenedIconPixelDatas, &ptDevVideoMem->tPixelDatas);
    }
    else
    {
        Press_button(&g_atDirAndFileLayout[iDirFileIndex]);
        Press_button(&g_atDirAndFileLayout[iDirFileIndex + 1]);
    }
}
...
static void DeSelectDirFileIcon(int iDirFileIndex)
{
    ...
    if (g_aptDirContents[iDirFileContentIndex]->eFileType == FILETYPE_DIR)
    {
        Pic_merge(g_atDirAndFileLayout[iDirFileIndex].iTopLeftX, g_atDirAndFileLayout[iDirFileIndex].iTopLeftY, &g_tDirClosedIconPixelDatas, &ptDevVideoMem->tPixelDatas);
    }
    else
    {
        Release_button(&g_atDirAndFileLayout[iDirFileIndex]);
        Release_button(&g_atDirAndFileLayout[iDirFileIndex + 1]);
    }
}
...
static void BrowsePageRun(P_Page_Params ptParentPageParams)
{
    int iIndex;
    ...
    /* 0. 获得要显示的目录的内容 */
    iError = Get_dir_contents(g_strCurDir, &g_aptDirContents, &g_iDirContentsNumber);
    if (iError)
    {
        ...
        if (tInputEvent.iPressure == 0)
        {
            ...
            if (bIconPressed)
            {
                if (iIndexPressed < DIRFILE_ICON_INDEX_BASE)  /* 菜单图标 */
                {
                    ...
                    {
                        Release_button(&g_atMenuIconsLayout[iIndexPressed]);
                    }
                ...
                else /* "目录和文件图标" */
                {
                    ...
                    else if (bHaveClickSelectIcon) /* 按下和松开都是同一个按钮, 并且"选择"按钮是按下状态 */
                    {
                        ...
                        if (g_aptDirContents[iDirFileContentIndex]->eFileType == FILETYPE_DIR)
                        {
                            Release_button(&g_atMenuIconsLayout[1]);  /* 同时松开"选择按钮" */
                            bHaveClickSelectIcon = 0;
                    ...
                    else  /* "选择"按钮不被按下时, 单击目录则进入 */
                    {
                        ...
                        else if (bUsedToSelectDir == 0) /* bUsedToSelectDir为0时单击文件时显示它 */
                        {
                            ...
                            if (Is_picture_file_supported(tPageParams.strCurPictureFile) != (-8))
                            {
                            ...
                            if (Is_picture_file_supported(tPageParams.strCurPictureFile) == (-8)){
                                ...
                                Get_font_ops("freetype")->Set_font_size(24);
                                ShowBrowsePage(&g_tBrowsePageMenuIconsLayout);
                            }
                            /*
                             * show novel
                    ...
                    if (iIndex < DIRFILE_ICON_INDEX_BASE)  /* 菜单图标 */
                    {
                        if (bUsedToSelectDir)
                        {
                            if (!(bHaveClickSelectIcon && (iIndexPressed == 1)))  /* 如果已经按下"选择"按钮, 自然不用再次反转该图标 */
            					Press_button(&g_atMenuIconsLayout[iIndex]);
                        }
                        else
                        {
...
static T_Page_action g_t_browse_page_action = {
        .name            = "browse",
        .Run             = BrowsePageRun,
        .Get_input_event = BrowsePageGetInputEvent,
...
{
    return Register_page_action(&g_t_browse_page_action);
}

...
static int is_JPG_format(P_File_Map ptFileMap);
static int Get_pixel_datas_frm_JPG(P_File_Map ptFileMap, P_Pixel_Datas ptPixelDatas);
static int Free_pixel_datas_for_JPG(P_Pixel_Datas ptPixelDatas);

static T_Pic_File_Parser  g_t_JPG_parser = {
        .name           = "jpg",
        .Is_support      = is_JPG_format,
        .Get_pixel_datas  = Get_pixel_datas_frm_JPG,
        .Free_pixel_datas = Free_pixel_datas_for_JPG,
};
...
static int Get_pixel_datas_frm_JPG(P_File_Map ptFileMap, P_Pixel_Datas ptPixelDatas)
{
    ...
    if(setjmp(tJerr.setjmp_buffer))
    {
        ...
        if (ptPixelDatas->auc_pixel_datas)
        {
    ...
    ptPixelDatas->auc_pixel_datas = malloc(ptPixelDatas->iTotalBytes);
...
{
    return Register_pic_file_parser(&g_t_JPG_parser);
}
